    ...
    <div className="max-w-xl mx-auto pb-28">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500">領収書・書類・清掃担当者・アカウント</p>
        </div>
      </div>

      {/* 事業者情報 */}
      <section className="mb-4">
        <div className="flex items-center gap-2 mb-2 px-1"><Building2 size={16} className="text-slate-600" /><h2 className="text-base font-bold text-slate-700 dark:text-slate-300">事業者情報</h2></div>
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm divide-y divide-slate-100 dark:divide-slate-700">
          {[{ key:'business_name',label:'事業者名・氏名',placeholder:'山田 太郎'},{key:'business_address',label:'事業者の住所',placeholder:'〇〇県〇〇市〇〇町1-2-3'},{key:'business_phone',label:'電話番号',placeholder:'090-0000-0000'}].map(({key,label,placeholder})=>(
            <div key={key} className="px-4 py-3.5">
              <label className="text-sm font-medium text-slate-600 dark:text-slate-400 block mb-1.5">{label}</label>
      ...
      {/* 駐車場情報 */}
      <section className="mb-4">
        <div className="flex items-center gap-2 mb-2 px-1"><Car size={16} className="text-slate-600" /><h2 className="text-base font-bold text-slate-700 dark:text-slate-300">駐車場情報</h2></div>
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm divide-y divide-slate-100 dark:divide-slate-700">
      ...
      {/* 書類設定 */}
      <section className="mb-4">
        <div className="flex items-center gap-2 mb-2 px-1"><FileText size={16} className="text-slate-600" /><h2 className="text-base font-bold text-slate-700 dark:text-slate-300">書類設定</h2></div>
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="px-4 py-3.5">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1.5">領収書番号の先頭に付く文字（例: R → R0001）</p>
          </div>
        </div>
      </section>
      ...
      {/* 清掃担当者 */}
      <section className="mb-4">
        <div className="flex items-center gap-2 mb-2 px-1"><Users size={16} className="text-slate-600" /><h2 className="text-base font-bold text-slate-700 dark:text-slate-300">清掃担当者リスト</h2></div>
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-4">
          <p className="text-xs text-slate-400 dark:text-slate-500 mb-3">清掃記録の入力時にプルダウンで選択できます</p>
          <div className="flex flex-wrap gap-2 mb-3 min-h-[28px]">
            {persons.length === 0 && <p className="text-xs text-slate-400 dark:text-slate-500">担当者が登録されていません</p>}
            {persons.map(name=>(
              <span key={name} className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-600 px-2.5 py-1 rounded-full text-xs font-medium">
                {name}
                <button onClick={()=>removePerson(name)} className="flex items-center justify-center w-7 h-7 rounded-full text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 -mr-1 ml-0.5 transition-colors"><X size={14}/></button>
              </span>
            ))}
          </div>
      ...
      {/* 最終ログイン */}
      {lastSignIn && (
        <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl px-4 py-3 mb-4 flex items-center justify-between">
          <p className="text-sm text-slate-500 dark:text-slate-400">最終ログイン日時</p>
          <p className="text-sm font-bold text-slate-700 dark:text-slate-300 tabular-nums">{lastSignIn}</p>
        </div>
      )}
      ...
      {/* 車庫証明の補足 */}
      <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 rounded-2xl p-3.5 mb-4">
        <p className="text-xs font-bold text-amber-800 dark:text-amber-400 mb-1.5">車庫証明について（補足）</p>
        <ul className="text-[11px] text-amber-700 dark:text-amber-400 space-y-1 list-disc list-inside leading-relaxed">
      ...
      {/* 保存ボタン */}
      <div className="save-bar bg-white/95 dark:bg-slate-800/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-700 p-3">
