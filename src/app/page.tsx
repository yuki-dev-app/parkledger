...
async function getData(ownerId: number) {
  const now    = new Date();
  const ym     = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
  const today  = now.toISOString().slice(0, 10);
  const in30   = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);

  const [garages, contractorCount, unpaidContractors, expiringContractors, newInquiries] = await Promise.all([
    sql`SELECT status FROM garages WHERE owner_id = ${ownerId}`,
    sql`SELECT COUNT(*) AS n FROM contractors WHERE archived_at = '' AND owner_id = ${ownerId}`,
    ...
    sql`
      ...
      WHERE c.archived_at = ''
        ...
        AND c.contract_end <= ${in30}
      ORDER BY c.contract_end
      LIMIT 5
    `,

    sql`
      SELECT name, created_at FROM inquiries
      WHERE status = 'new' AND owner_id = ${ownerId}
    ...
    <div className="space-y-4 max-w-2xl mx-auto">
      ...
      {d.unpaid.length > 0 && (
        <div className="bg-white border-2 border-red-400 rounded-2xl overflow-hidden">
          <div className="bg-red-500 px-4 py-3 flex items-center gap-2">
            ...
            <p className="text-white font-bold text-base">入金がまだの方（{d.unpaid.length}名）</p>
          </div>
              ...
              <div key={i} className="px-4 py-4 flex items-center justify-between gap-3">
                ...
                <div className="shrink-0">
                  {c.phone ? (
                    ...
                    <Link href="/payments" className="flex items-center gap-1.5 bg-slate-700 text-white px-4 py-3 rounded-xl font-bold text-sm">
                      確認する
                    </Link>
                  )}
                </div>
          ...
          <div className="px-4 pb-4">
            <Link href="/payments"
              ...
              {d.unpaid.length > 2 ? `他 ${d.unpaid.length - 2} 名を含む — 入金チェックへ` : '入金チェック画面を開く'}
      ...
      {d.expiring.length > 0 && (
        <div className="bg-white border-2 border-amber-400 rounded-2xl overflow-hidden">
          <div className="bg-amber-400 px-4 py-3 flex items-center gap-2">
            <AlertTriangle size={18} className="text-white" />
            <p className="text-white font-bold text-base">契約の期限が近い方（{d.expiring.length}名）</p>
          </div>
          <div className="divide-y divide-slate-100">
            {d.expiring.map((c, i) => (
              <div key={i} className="px-4 py-4 flex items-center justify-between gap-3">
      ...
      {d.inquiries.length > 0 && (
        <div className="bg-white border-2 border-blue-400 rounded-2xl overflow-hidden">
          <div className="bg-blue-500 px-4 py-3 flex items-center gap-2">
            ...
            <p className="text-white font-bold text-base">問い合わせが届いています（{d.inquiries.length}件）</p>
          </div>
          <div className="divide-y divide-slate-100">
            {d.inquiries.map((inq, i) => (
              <div key={i} className="px-4 py-4 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-slate-500 mt-0.5">{new Date(inq.created_at).toLocaleDateString('ja-JP')}</p>
                </div>
                <Link href="/inquiries"
      ...
      {d.total > 0 && (
        <div className="grid grid-cols-3 gap-2">
          <Link href="/garages" className="bg-white border border-slate-200 rounded-2xl p-4 text-center hover:bg-slate-50 active:bg-slate-100">
          ...
          <Link href="/contractors" className="bg-white border border-slate-200 rounded-2xl p-4 text-center hover:bg-slate-50 active:bg-slate-100">
          ...
          <Link href="/payments"
            className={`border rounded-2xl p-4 text-center active:scale-95 transition-transform ${
              allPaid ? 'bg-emerald-50 border-emerald-200 hover:bg-emerald-100' : 'bg-red-50 border-red-200 hover:bg-red-100'
            }`}>
            <CreditCard size={20} className={`mx-auto mb-1.5 ${allPaid ? 'text-emerald-500' : 'text-red-500'}`} />
            <p className={`text-2xl font-black tabular-nums ${allPaid ? 'text-emerald-700' : 'text-red-600'}`}>{d.unpaid.length}</p>
